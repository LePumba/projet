...
?>
<div class="form-login" style="margin-left: 30rem">
    <h2>Article:</h2>
    ...
    <div class="article">
        ...
        <span><?= strftime('%d/%m/%Y %H:%M', strtotime($article->getCreatedAt())); ?></span>
        ...
        foreach($article->fetchComments(DB::getDbLink()) as $comment) { ?>
            <div class="comment">
                <p class="comment-content">
                    <?= html_entity_decode($comment->getTextContent()); ?>
                </p>
                <span class="author"></span>
                <span class="comment-date">
                    <?= strftime('%d/%m/%Y %H:%M', strtotime($comment->getDateCreated())); ?>
